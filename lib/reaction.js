...
export var
  ...
  ,forceReschedule = () => isFlushing && (reschedule = true)
...
function schedule (subscribers) {
  ...
  while (item) {
    ...
    if (value.state != states.target) (
      ((!isFlushing || reschedule) && (value.state == states.clean)) && stack.push(value)
...
function flush () {
  ...
  var
    queueIdx = 0 ,currentQueue ,currentLength ,lastPrevIdx
  ...
  queuesLoop: for (;queueIdx < queuesSize ;queueIdx++) {
    currentQueue = queues[queueIdx].sort((a ,b) => a.lvl - b.lvl)

    if (queueIdx) lastPrevIdx = effectIdx - 1

    for (effectIdx = 0 ,currentLength = currentQueue.length ;effectIdx < currentLength ;effectIdx++) {
      effect = currentQueue[effectIdx]

      if (effect.state == states.dirty) resolveUpstreams(effect.obs)

      if (reschedule) break queuesLoop
  ...
  if (reschedule) return (
    (currentQueue == immediateQueue)
      ? immediateQueue.splice(0 ,effectIdx)
      : (immediateQueue.splice(0 ,lastPrevIdx) ,queue.splice(0 ,effectIdx))

    ,(reschedule = false)

    ,flush()
  )
  else immediateQueue.length = queue.length = 0
}
